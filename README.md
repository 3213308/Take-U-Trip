...
用户输入自然语言需求后，右侧面板实时展示多 Agent 协作过程（加载记忆 → 意图识别 → 规划行程 → 核算预算 → 审查质量 → 生成结果）。

**第 2 轮规划被 Reviewer 打回修订**——Reviewer 不盲信模型自报 decision，改为确定性统计 blocker 数量：

![聊天与 Agent 执行过程（打回修订）](docs/screenshot-chat-review.png)

重规划后**第 3 轮通过**，全程共调用 35 个真实工具（天气/景点/酒店/餐饮/12306/路径规划）：

![重规划通过](docs/screenshot-chat-pass.png)

### 行程详情与预算核算

Reviewer 通过后生成完整行程：每日时间线、分类预算（交通/住宿/餐饮/门票）、**12306 真实车次信息**（多端点轮换查询）、天气与交通提醒。Budgeter 用代码重算费用，不相信模型自填数字。

![行程详情与预算](docs/screenshot-itinerary.png)

### 地图导航

行程地点自动落图，支持按日切换、在地图上定位每个活动：

![地图导航](docs/screenshot-map.png)

### 愿望清单

收藏想去的地点，可从行程导入、手动打卡：

![愿望清单](docs/screenshot-wishlist.png)
